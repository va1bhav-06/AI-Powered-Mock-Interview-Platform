...
export const transcribeOnly = async (req, res, next) => {
  try {
    // 1. Check if the file exists AND that it actually contains audio data bytes
    if (!req.file || !req.file.buffer || req.file.buffer.length === 0) {
      return res.status(400).json({ 
        success: false, 
        message: 'Audio file received is empty (0 bytes). Please check microphone recording.' 
      });
    }

    // 2. Pass the data cleanly down to your service function
    ...
    console.error("Controller transcription error:", error.message);
    next(error);
  }
};
