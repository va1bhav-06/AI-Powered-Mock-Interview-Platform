...
function InterviewPage() {
  ...
  setInterviewerState(STATE_THINKING);
  try {
    // 1. Determine the extension based on the blob type (safeguard for Safari/Chrome)
    const extension = audioBlob.type.includes('mp4') ? 'mp4' : 'webm';
    
    // 2. Wrap the blob into a real File object with an explicit audio filename extension
    const audioFile = new File([audioBlob], `recording.${extension}`, { 
      type: audioBlob.type 
    });

    // 3. Pass the named file into your transcription function instead of the raw blob
    ...
    console.error("Submission breakdown details:", error);
    toast.error(error.response?.data?.message || "Failed to submit answer");
    setInterviewerState(STATE_LISTENING);
  } finally {
    setSubmitting(false);
